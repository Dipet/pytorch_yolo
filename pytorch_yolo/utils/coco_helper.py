...
def create_image_info(file_name, height, width,
                      license_id=1, coco_url='',
                      date_captured='', flickr_url='', id=1):
...
def create_annotation_info(image_id: int, bbox: list,
                           segmentation=(), area=0, iscrowd=0,
                           category_id=1, id=1):
    return {'segmentation': list(segmentation),
            'area': area,
            'iscrowd': iscrowd,
            'image_id': image_id,
            'bbox': bbox,
...
def dataset_from_dict(data: dict, categories=(('car', 'vehicle'),), image_dir='', has_image_info=False):
    ...
    category_info = [create_category(id=i, name=cat, supercategory=sup) for
                     i, (cat, sup) in enumerate(categories)]
    ...
    for i, (path, item) in tqdm(enumerate(data.items()), total=len(data)):
        if not has_image_info:
            ...
            images_info.append(create_image_info(path,
                                                 image.height,
                                                 image.width,
                                                 id=i))
        else:
        ...
        for item_info in item:
            ...
            annotation_info.append(create_annotation_info(id=anno_id, bbox=bbox,
                                                          image_id=i,
                                                          category_id=item_info['type']))
            anno_id += 1
...
def create_result_info(image_id: int, category_id: int,
                       bbox: list, score: float):
