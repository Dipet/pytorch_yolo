...
class YOLOv3SPP(YOLOBase):
    def __init__(self, *args, **kwargs):
        ...
        self.sequence_spp.add_module(
            "conv1", ConvBlock(self.down[-1].out_channels, 512 // kd, size=1, stride=1)
        )
        self.sequence_spp.add_module(
            "conv2",
            ConvBlock(self.sequence_spp[-1].out_channels, 1024 // kd, size=3, stride=1),
        )
        self.sequence_spp.add_module(
            "conv3",
            ConvBlock(self.sequence_spp[-1].out_channels, 512 // kd, size=1, stride=1),
        )
        ...
        self.branch1_1.add_module(
            "conv1",
            ConvBlock(
                self.sequence_spp[-1].out_channels * 4, 512 // kd, size=1, stride=1
            ),
        )
        self.branch1_1.add_module(
            "conv2",
            ConvBlock(self.branch1_1[-1].out_channels, 1024 // kd, size=3, stride=1),
        )
        self.branch1_1.add_module(
            "conv3",
            ConvBlock(self.branch1_1[-1].out_channels, 512 // kd, size=1, stride=1),
        )
        ...
        self.branch2_1 = nn.Sequential(
            ConvBlock(self.branch1_1[-1].out_channels, 256 // kd, size=1, stride=1),
            Upsample(2),
        )
        ...
        self.branch2_2.add_module(
            "conv1",
            ConvBlock(
                self.branch2_1[0].out_channels + self.down[3].out_channels,
                256 // kd,
                size=1,
                stride=1,
            ),
        ...
        self.branch2_2.add_module(
            "conv2",
            ConvBlock(self.branch2_2[-1].out_channels, 512 // kd, size=3, stride=1),
        )
        self.branch2_2.add_module(
            "conv3",
            ConvBlock(self.branch2_2[-1].out_channels, 256 // kd, size=1, stride=1),
        )
        self.branch2_2.add_module(
            "conv4",
            ConvBlock(self.branch2_2[-1].out_channels, 512 // kd, size=3, stride=1),
        ...
        self.branch3_1 = nn.Sequential(
            ConvBlock(self.branch2_2[-1].out_channels, 128 // kd, size=1, stride=1),
            Upsample(2),
        )
        ...
        self.branch3_2.add_module(
            "conv1",
            ConvBlock(
                self.branch3_1[0].out_channels + self.down[2].out_channels,
                128 // kd,
                size=1,
                stride=1,
            ),
        ...
        self.branch3_2.add_module(
            "conv2",
            ConvBlock(self.branch3_2[-1].out_channels, 256 // kd, size=3, stride=1),
        )
        self.branch3_2.add_module(
            "conv3",
            ConvBlock(self.branch3_2[-1].out_channels, 128 // kd, size=1, stride=1),
        )
        self.branch3_2.add_module(
            "conv4",
            ConvBlock(self.branch3_2[-1].out_channels, 256 // kd, size=3, stride=1),
